...
export const ManagerProductImageCard=({data,productId,update})=>{
  ...
  const deleteTemplateProductImage=useCallback(async (productId,id) => { 
    try{
      await request(`https://morning-production-app.herokuapp.com/api/product/image/repository/delete/template/${productId}/${id}`, 'POST', null,{
